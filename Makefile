...
# Windows development environment uses the protoc tool installed with the
# project code-generation toolchain.
PROTOC ?= C:/Users/zzhjz/go/bin/protoc.exe
PROTOC_GEN_GO ?= C:/Users/zzhjz/go/bin/protoc-gen-go.exe
PROTOC_GEN_GO_GRPC ?= C:/Users/zzhjz/go/bin/protoc-gen-go-grpc.exe
...
gen-rpc: # Generate RPC files from proto | 生成 RPC 的代码
	$(PROTOC) --plugin=protoc-gen-go=$(PROTOC_GEN_GO) --plugin=protoc-gen-go-grpc=$(PROTOC_GEN_GO_GRPC) --proto_path=desc --go_out=./travel --go_opt=paths=source_relative --go-grpc_out=./travel --go-grpc_opt=paths=source_relative desc/$(SERVICE_STYLE).proto
