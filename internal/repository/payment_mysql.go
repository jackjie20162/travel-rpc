...
func NewPaymentRepository(client *ent.Client) PaymentRepository {
	return &mysqlPaymentRepository{client: client}
}

func (r *mysqlPaymentRepository) Create(ctx context.Context, input CreatePaymentInput) (*PaymentRecord, error) {
	if input.TenantID <= 0 || input.MerchantID <= 0 || input.OrderNo == "" || input.Provider == "" || input.IdempotencyKey == "" {
	...
	if err == nil {
		return r.toRecord(existing, input.OrderNo), nil
	}
	...
	if o.Status != "PENDING_PAYMENT" && o.Status != "PAYMENT_PROCESSING" {
		return nil, &ErrPaymentConflict{}
	}
	p, err := r.client.Payment.Create().SetTenantID(input.TenantID).SetMerchantID(input.MerchantID).SetOrderID(int64(o.ID)).SetPaymentNo(newPaymentNo()).SetProvider(input.Provider).SetAmount(o.TotalAmount).SetCurrency(o.Currency).SetStatus("CREATED").SetIdempotencyKey(input.IdempotencyKey).Save(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *mysqlPaymentRepository) Get(ctx context.Context, tenantID, merchantID int64, paymentNo string) (*PaymentRecord, error) {
	...
	o, err := r.client.Order.Get(ctx, int(p.OrderID))
	if err != nil || o.TenantID != tenantID || o.MerchantID != merchantID {
		return nil, &ErrPaymentNotFound{}
	}
	return r.toRecord(p, o.OrderNo), nil
}
...
func (r *mysqlPaymentRepository) SetProviderID(ctx context.Context, tenantID, merchantID int64, paymentNo, providerPaymentID string) (*PaymentRecord, error) {
	...
	if p.Status == "PAID" && p.ProviderPaymentID != providerPaymentID {
		return nil, &ErrPaymentConflict{}
	}
	if p.ProviderPaymentID != "" && p.ProviderPaymentID != providerPaymentID {
		return nil, &ErrPaymentConflict{}
	}
	p, err = r.client.Payment.UpdateOneID(p.ID).SetProviderPaymentID(providerPaymentID).SetStatus("PROCESSING").Save(ctx)
	if err != nil {
		return nil, err
	}
	o, err := r.client.Order.Get(ctx, int(p.OrderID))
...
func (r *mysqlPaymentRepository) MarkPaid(ctx context.Context, tenantID, merchantID int64, paymentNo, providerPaymentID string) (*PaymentRecord, error) {
	...
	if p.Status == "PAID" {
		...
		o, err := tx.Order.Get(ctx, int(p.OrderID))
		if err != nil {
			return nil, err
		}
	...
	p, err = tx.Payment.UpdateOneID(p.ID).SetProviderPaymentID(providerPaymentID).SetStatus("PAID").Save(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *mysqlPaymentRepository) toRecord(p *ent.Payment, orderNo string) *PaymentRecord {
	return &PaymentRecord{ID: int64(p.ID), PaymentNo: p.PaymentNo, OrderNo: orderNo, Provider: p.Provider, ProviderPaymentID: p.ProviderPaymentID, Amount: p.Amount, Currency: p.Currency, Status: p.Status}
}
