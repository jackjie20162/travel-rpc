...
func (r *mysqlPaymentRepository) Create(ctx context.Context, input CreatePaymentInput) (*PaymentRecord, error) {
    ...
    existing, err := r.client.Payment.Query().Where(
        payment.TenantIDEQ(input.TenantID), payment.MerchantIDEQ(input.MerchantID), payment.IdempotencyKeyEQ(input.IdempotencyKey),
    ).Only(ctx)
    ...
    p, err := r.client.Payment.Create().
        SetTenantID(input.TenantID).SetMerchantID(input.MerchantID).SetOrderID(o.ID).
        SetPaymentNo(newPaymentNo()).SetProvider(input.Provider).SetAmount(o.TotalAmount).
        SetCurrency(o.Currency).SetStatus("CREATED").SetIdempotencyKey(input.IdempotencyKey).Save(ctx)
...
func (r *mysqlPaymentRepository) Get(ctx context.Context, tenantID, merchantID int64, paymentNo string) (*PaymentRecord, error) {
    ...
    if err != nil || o.TenantID != tenantID || o.MerchantID != merchantID { return nil, &ErrPaymentNotFound{} }
    return r.toRecord(p, o.OrderNo), nil
}
