...
func (r *mysqlInventoryRepository) Check(ctx context.Context, tenantID, packageID int64, serviceDate, timeSlot string, quantity int) (*InventoryAvailability, error) {
	if tenantID <= 0 || packageID <= 0 || serviceDate == "" || quantity <= 0 {
		return nil, &ErrInvalidReservation{}
	}
	q := r.client.Inventory.Query().Where(
	...
	if timeSlot != "" {
		q = q.Where(inventory.TimeSlotEQ(timeSlot))
	}
	item, err := q.Only(ctx)
	if err != nil {
		return nil, err
...
func (r *mysqlInventoryRepository) Reserve(ctx context.Context, tenantID, merchantID, packageID int64, serviceDate, timeSlot string, quantity int, reservationKey string) (*ReservationResult, error) {
	if tenantID <= 0 || packageID <= 0 || serviceDate == "" || quantity <= 0 || reservationKey == "" {
		return nil, &ErrInvalidReservation{}
	}
	...
	if err == nil {
		// If merchantID is provided, verify it matches
		if merchantID > 0 && existing.MerchantID != merchantID {
			_ = tx.Rollback()
			return nil, &ErrReservationKeyConflict{}
		}
		if existing.Quantity != quantity || existing.Status == "RELEASED" || existing.Status == "EXPIRED" {
			_ = tx.Rollback()
			return nil, &ErrReservationKeyConflict{}
		}
		...
		if int64(inv.PackageID) != packageID || inv.ServiceDate != serviceDate {
			_ = tx.Rollback()
			return nil, &ErrReservationKeyConflict{}
		}
		if timeSlot != "" && inv.TimeSlot != timeSlot {
			_ = tx.Rollback()
			return nil, &ErrReservationKeyConflict{}
		}
		result = &ReservationResult{Reservation: existing, Remaining: inv.Capacity - inv.Reserved, UnitPrice: inv.UnitPrice, Currency: inv.Currency, InventoryMerchantID: inv.MerchantID}
	...
	// Do NOT filter by merchantID: public orders may book inventory owned by any merchant.
	// When timeSlot is empty, match any time slot for the given date
	q := tx.Inventory.Query().Where(inventory.TenantIDEQ(tenantID), inventory.PackageIDEQ(packageID), inventory.ServiceDateEQ(serviceDate), inventory.StatusEQ("OPEN"))
	if timeSlot != "" {
		q = q.Where(inventory.TimeSlotEQ(timeSlot))
	}
	item, err := q.Only(ctx)
	...
	if item.Capacity-item.Reserved < quantity {
		_ = tx.Rollback()
		return nil, &ErrInsufficientInventory{Remaining: item.Capacity - item.Reserved}
	}
	...
	hold, err := tx.InventoryReservation.Create().SetTenantID(tenantID).SetMerchantID(item.MerchantID).SetInventoryID(int64(updated.ID)).SetReservationKey(reservationKey).SetQuantity(quantity).SetStatus("RESERVED").SetExpiresAt(time.Now().Add(15 * time.Minute)).Save(ctx)
	if err != nil {
		_ = tx.Rollback()
		return nil, err
	}
	result = &ReservationResult{Reservation: hold, Remaining: updated.Capacity - updated.Reserved, UnitPrice: updated.UnitPrice, Currency: updated.Currency, InventoryMerchantID: updated.MerchantID}
	if err = tx.Commit(); err != nil {
		return nil, err
	}
	return result, nil
...
func (r *mysqlInventoryRepository) ConfirmReservation(ctx context.Context, tenantID, reservationID, orderID int64) error {
	if tenantID <= 0 || reservationID <= 0 || orderID <= 0 {
...
func (r *mysqlInventoryRepository) ReleaseReservation(ctx context.Context, tenantID, reservationID int64) error {
	...
	item, err := tx.Inventory.Get(ctx, int(hold.InventoryID))
	if err != nil {
		_ = tx.Rollback()
		return err
	}
	...
	if _, err = tx.InventoryReservation.UpdateOneID(hold.ID).SetStatus("RELEASED").Save(ctx); err != nil {
		_ = tx.Rollback()
		return err
	}
	return tx.Commit()
...
func (r *mysqlInventoryRepository) ExpireReservations(ctx context.Context, nowUnix int64, limit int) (int, error) {
	...
	for _, hold := range items {
		if err := r.ReleaseReservation(ctx, hold.TenantID, int64(hold.ID)); err == nil {
			count++
		}
	}
	return count, nil
...
func availability(item *ent.Inventory) *InventoryAvailability {
	return &InventoryAvailability{InventoryID: int64(item.ID), Remaining: item.Capacity - item.Reserved, UnitPrice: item.UnitPrice, Currency: item.Currency}
}
...
func (e *ErrReservationKeyConflict) Error() string {
	return "reservation key conflicts with an existing reservation"
}
