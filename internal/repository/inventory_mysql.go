...
func (r *mysqlInventoryRepository) Check(ctx context.Context, tenantID, packageID int64, serviceDate, timeSlot string, quantity int) (*InventoryAvailability, error) {
	...
	item, err := r.client.Inventory.Query().Where(
		inventory.TenantIDEQ(tenantID), inventory.PackageIDEQ(packageID),
		inventory.ServiceDateEQ(serviceDate), inventory.TimeSlotEQ(timeSlot), inventory.StatusEQ("OPEN"),
	).Only(ctx)
...
func (r *mysqlInventoryRepository) Reserve(ctx context.Context, tenantID, merchantID, packageID int64, serviceDate, timeSlot string, quantity int, reservationKey string) (*ReservationResult, error) {
	...
	if err == nil {
		if existing.MerchantID != merchantID || existing.Quantity != quantity || existing.Status == "RELEASED" || existing.Status == "EXPIRED" {
			_ = tx.Rollback()
		...
		if int64(inv.PackageID) != packageID || inv.ServiceDate != serviceDate || inv.TimeSlot != timeSlot {
			_ = tx.Rollback()
			return nil, &ErrReservationKeyConflict{}
		}
	...
	item, err := tx.Inventory.Query().Where(
		inventory.TenantIDEQ(tenantID), inventory.MerchantIDEQ(merchantID), inventory.PackageIDEQ(packageID),
		inventory.ServiceDateEQ(serviceDate), inventory.TimeSlotEQ(timeSlot), inventory.StatusEQ("OPEN"),
	).Only(ctx)
	...
	updated, err := tx.Inventory.UpdateOneID(item.ID).Where(
		inventory.ReservedEQ(item.Reserved), inventory.StatusEQ("OPEN"),
	).SetReserved(item.Reserved+quantity).Save(ctx)
	if err != nil { _ = tx.Rollback(); return nil, err }
	hold, err := tx.InventoryReservation.Create().SetTenantID(tenantID).SetMerchantID(merchantID).
		SetInventoryID(updated.ID).SetReservationKey(reservationKey).SetQuantity(quantity).
		SetStatus("RESERVED").SetExpiresAt(time.Now().Add(15*time.Minute)).Save(ctx)
	if err != nil { _ = tx.Rollback(); return nil, err }
...
func (r *mysqlInventoryRepository) ConfirmReservation(ctx context.Context, tenantID, reservationID, orderID int64) error {
	...
	_, err := r.client.InventoryReservation.UpdateOneID(int(reservationID)).
		Where(inventoryreservation.TenantIDEQ(tenantID), inventoryreservation.StatusEQ("RESERVED"), inventoryreservation.ExpiresAtGT(time.Now())).
...
func (r *mysqlInventoryRepository) ExpireReservations(ctx context.Context, nowUnix int64, limit int) (int, error) {
	...
	count := 0
	for _, hold := range items { if err := r.ReleaseReservation(ctx, hold.TenantID, hold.ID); err == nil { count++ } }
	return count, nil
}

func availability(item *ent.Inventory) *InventoryAvailability {
	return &InventoryAvailability{InventoryID:item.ID, Remaining:item.Capacity-item.Reserved, UnitPrice:item.UnitPrice, Currency:item.Currency}
}
