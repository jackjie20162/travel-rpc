...
func (r *mysqlBookingRepository) CreateFromReservation(ctx context.Context, reservationID int64, input CreateOrderInput) (*ent.Order, error) {
    ...
    hold, err := tx.InventoryReservation.Query().Where(
        inventoryreservation.IDEQ(int(reservationID)),
        inventoryreservation.TenantIDEQ(input.TenantID),
        inventoryreservation.MerchantIDEQ(input.MerchantID),
    ).Only(ctx)
    if err != nil { return nil, err }
    ...
    inv, err := tx.Inventory.Get(ctx, int(hold.InventoryID))
    if err != nil { return nil, err }
    if inv.TenantID != input.TenantID || inv.MerchantID != input.MerchantID || int64(inv.PackageID) != input.PackageID || inv.ServiceDate != input.ServiceDate || inv.TimeSlot != input.TimeSlot || inv.Status != "OPEN" {
        return nil, &ErrReservationKeyConflict{}
    }

    prod, err := tx.Product.Get(ctx, int(input.ProductID))
    if err != nil { return nil, err }
    if prod.TenantID != input.TenantID || prod.MerchantID != input.MerchantID || prod.Status != "ACTIVE" || prod.Currency != inv.Currency {
        return nil, &ErrReservationKeyConflict{}
    }
    ...
    builder := tx.Order.Create().SetTenantID(input.TenantID).SetMerchantID(input.MerchantID).
        SetOrderNo(newOrderNo()).SetTotalAmount(total).SetCurrency(inv.Currency).
    ...
    if _, err = tx.OrderItem.Create().SetOrderID(order.ID).SetProductID(int(input.ProductID)).SetPackageID(int(input.PackageID)).
        SetQuantity(input.Quantity).SetUnitPrice(inv.UnitPrice).SetTotalAmount(total).
