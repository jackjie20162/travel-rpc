...
type PaymentService struct { travel.UnimplementedPaymentServiceServer; payments repository.PaymentRepository }
...
func (s *PaymentService) Create(ctx context.Context, req *travel.CreatePaymentRequest) (*travel.Payment, error) {
    ...
    if err != nil { return nil, mapPaymentError(err) }; return toPayment(p), nil
}
...
func (s *PaymentService) Get(ctx context.Context, req *travel.PaymentNoRequest) (*travel.Payment, error) {
    ...
    tenantID, err := auth.TenantID(ctx); if err != nil { return nil, status.Error(codes.Unauthenticated, err.Error()) }
    merchantID, err := auth.MerchantID(ctx); if err != nil { return nil, status.Error(codes.Unauthenticated, err.Error()) }
    p, err := s.payments.Get(ctx, tenantID, merchantID, req.GetPaymentNo()); if err != nil { return nil, mapPaymentError(err) }; return toPayment(p), nil
}

func (s *PaymentService) SetProviderID(ctx context.Context, req *travel.SetPaymentProviderIDRequest) (*travel.Payment, error) {
    ...
    tenantID, err := auth.TenantID(ctx); if err != nil { return nil, status.Error(codes.Unauthenticated, err.Error()) }
    merchantID, err := auth.MerchantID(ctx); if err != nil { return nil, status.Error(codes.Unauthenticated, err.Error()) }
    p, err := s.payments.SetProviderID(ctx, tenantID, merchantID, req.GetPaymentNo(), req.GetProviderPaymentId()); if err != nil { return nil, mapPaymentError(err) }; return toPayment(p), nil
}
...
func (s *PaymentService) MarkPaid(ctx context.Context, req *travel.MarkPaymentPaidRequest) (*travel.Payment, error) {
    ...
    tenantID, err := auth.TenantID(ctx); if err != nil { return nil, status.Error(codes.Unauthenticated, err.Error()) }
    merchantID, err := auth.MerchantID(ctx); if err != nil { return nil, status.Error(codes.Unauthenticated, err.Error()) }
    p, err := s.payments.MarkPaid(ctx, tenantID, merchantID, req.GetPaymentNo(), req.GetProviderPaymentId()); if err != nil { return nil, mapPaymentError(err) }; return toPayment(p), nil
}

func toPayment(p *repository.PaymentRecord) *travel.Payment { return &travel.Payment{Id:p.ID, PaymentNo:p.PaymentNo, OrderNo:p.OrderNo, Provider:p.Provider, ProviderPaymentId:p.ProviderPaymentID, Amount:p.Amount, Currency:p.Currency, Status:p.Status} }
func mapPaymentError(err error) error { switch err.(type) { case *repository.ErrPaymentNotFound: return status.Error(codes.NotFound, err.Error()); case *repository.ErrPaymentConflict: return status.Error(codes.Aborted, err.Error()); default: return status.Error(codes.Internal, err.Error()) } }
