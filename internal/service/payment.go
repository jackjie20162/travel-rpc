...
func (s *PaymentService) Create(ctx context.Context, req *travel.CreatePaymentRequest) (*travel.Payment, error) {
	if req == nil || req.GetOrderNo() == "" || req.GetProvider() == "" || req.GetIdempotencyKey() == "" {
		return nil, status.Error(codes.InvalidArgument, "order number, provider and idempotency key are required")
	}
...
func (s *PaymentService) Get(ctx context.Context, req *travel.PaymentNoRequest) (*travel.Payment, error) {
	if req == nil || req.GetPaymentNo() == "" {
		return nil, status.Error(codes.InvalidArgument, "payment number is required")
	}
	tenantID, err := auth.TenantID(ctx)
	if err != nil {
		return nil, status.Error(codes.Unauthenticated, err.Error())
...
func (s *PaymentService) SetProviderID(ctx context.Context, req *travel.SetPaymentProviderIDRequest) (*travel.Payment, error) {
	if req == nil || req.GetPaymentNo() == "" || req.GetProviderPaymentId() == "" {
		return nil, status.Error(codes.InvalidArgument, "payment number and provider payment id are required")
	}
	tenantID, err := auth.TenantID(ctx)
	if err != nil {
		return nil, status.Error(codes.Unauthenticated, err.Error())
	...
	p, err := s.payments.SetProviderID(ctx, tenantID, merchantID, req.GetPaymentNo(), req.GetProviderPaymentId())
	if err != nil {
		return nil, mapPaymentError(err)
	}
	return toPayment(p), nil
...
func mapPaymentError(err error) error {
	...
	case *repository.ErrPaymentConflict:
